// zeno.js — Updated, fixed, and smoothed for Zeno MD Bot

const { File: BufferFile } = require('node:buffer');
global.File = BufferFile;

// ✅ Lucifer Zeno Inc Property 2025
const baileys = require('@luciferzeno/baileys');
const {
    makeWASocket,
...
const globalContextInfo = {
    ...
    forwardedNewsletterMessageInfo: {
        ...
        newsletterName: '◢◤ Lucifer Zeno Nexus ◢◤',
        serverMessageId: 144
    }
};
...
async function setupSession() {
    ...
    if (!fs.existsSync(sessionPath)) {
        if (!config.SESSION_ID || !config.SESSION_ID.startsWith('Silva~')) {
            throw new Error('Invalid or missing SESSION_ID. Must start with Zeno~');
        }
        logMessage('INFO', '⬇ Downloading session from Mega.nz...');
        const megaCode = config.SESSION_ID.replace('Zeno~', '');
...
function generateFancyBio() {
    ...
    const bios = [
        ...
        `⚡ Zeno MD Active ✦ ${timeStr} ✦ ${dateStr} ✦`,
        `💫 ${config.BOT_NAME} Operational ✦ ${dateStr} ✦`,
        `🚀 Zeno MD Live ✦ ${dateStr} ✦ ${timeStr} ✦`,
...
async function sendWelcomeMessage(sock) {
    ...
    const welcomeMsg = `*Hello ✦ ${config.BOT_NAME} ✦ User!*\n\n` +
        `✅ Zeno MD Bot is now active!\n\n` +
        `*Prefix:* ${prefix}\n` +
        `*Mode:* ${config.MODE}\n` +
        `*Plugins Loaded:* ${plugins.size}\n\n` +
        `*⚙️ Configuration Status:*\n\`\`\`${configTable}\`\`\`\n\n` +
        `*Description:* ${config.DESCRIPTION}\n\n` +
        `⚡ Powered by Lucifer Zeno Inc\nGitHub: https://github.com/vishalkumar67/Lucifer-Zeno-MD`;
        ...
        await sock.sendMessage(sock.user.id, {
            ...
            contextInfo: {
                ...
                externalAdReply: {
                    ...
                    thumbnailUrl: "https://files.catbox.moe/x4xxwc.jpg",
        ...
        try {
            ...
            for (const m of messages) {
                ...
                if (command === 'menu') {
                    ...
                    await sock.sendMessage(sender, {
                        image: { url: 'https://files.catbox.moe/x4xxwc.jpg' },
                        caption: menuText,
                        contextInfo: {
                            ...globalContextInfo,
                            externalAdReply: {
...
(async () => {
    try {
        logMessage('INFO', 'Booting Zeno MD Bot...');
